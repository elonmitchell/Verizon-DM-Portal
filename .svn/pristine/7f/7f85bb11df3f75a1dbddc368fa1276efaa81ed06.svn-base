...
(function($)
{
	...
	p.onClick = function(e)
	{
		...
		if(target.is(CSS_DOT_SUGGESTION) || target.is(CSS_DOT_LABEL)) {
			for (var i = 0 ; i < self._suggestions.length; i++) {
				var suggestedTag = self._suggestions[i];
				if (self.itemManager().itemToString(suggestedTag) == target.text()) {
					self.trigger('onAddItem', suggestedTag);
					break;
				}
			}
		}
	...
	p.selectFromDropdown = function()
	{
		...
		if(suggestion)
		{
			...
			self.trigger('onAddItem', suggestion);
			if (self.core().hasPlugin('tags')) {
				self.val('');
